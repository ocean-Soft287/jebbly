...
class MapCubit extends Cubit<MapState> {
  ...
  Future<void> getLocationUpdates() async {
    ...
    location.PermissionStatus permissionGranted = await _locationController.hasPermission();
    if (permissionGranted == location.PermissionStatus.denied) {
      permissionGranted = await _locationController.requestPermission();
      if (permissionGranted != location.PermissionStatus.granted) return;
    }

    _locationController.onLocationChanged.listen((location.LocationData currentLocation) async {
      if (isTrackingLocation && currentLocation.latitude != null && currentLocation.longitude != null) {
  ...
  Future<void> detectMyLocation() async {
    ...
    try {
      location.LocationData currentLocation = await _locationController.getLocation();
  ...
  Future<void> searchLocation(String? locationQuery) async {
    try {
      ...
      if (locations.isNotEmpty) {
        ...
        mapController?.animateCamera(CameraUpdate.newLatLngZoom(newLocation, 12));
